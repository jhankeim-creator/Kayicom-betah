...
class SiteSettings(BaseModel):
    ...
    payerurl_public_key: Optional[str] = None
    payerurl_secret_key: Optional[str] = None
...
async def create_order(order_data: OrderCreate, user_id: str, user_email: str):
    ...
    if order_data.payment_method == "payerurl":
        settings = await db.settings.find_one({"id": "site_settings"})
        pub_key = (settings or {}).get("payerurl_public_key", "")
        sec_key = (settings or {}).get("payerurl_secret_key", "")
...
async def _release_due_escrows():
    ...
    Handles two cases:
    1. Buyer confirmed delivery → release after escrow_release_at (3 days post-confirm)
    ...
    cursor = db.orders.find({
        "$or": [
            ...
            {
                "escrow_status": "held",
                "escrow_held_at": {"$lte": auto_release_cutoff},
            },
        ]
    }, {"_id": 0})
