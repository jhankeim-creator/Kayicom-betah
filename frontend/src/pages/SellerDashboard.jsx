...
const getCatMeta = (cat) => CATEGORY_META[cat] || { label: cat, icon: Package, color: 'from-gray-500/20 to-gray-500/20 border-gray-500/30', badge: 'bg-gray-500/20 text-gray-300' };
    ...
    <div className="min-h-screen bg-[#0a0a0a]">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="flex gap-1 mt-1 flex-wrap">
                            ...
                            <Badge className={status === 'approved' ? 'bg-green-500/20 text-green-300' : status === 'rejected' ? 'bg-red-500/20 text-red-300' : 'bg-yellow-500/20 text-yellow-300'}>
                              {status === 'pending_review' ? 'Pending Review' : status.replace('_', ' ')}
                            </Badge>
                          </div>
            ...
            <div className="rounded-xl bg-[#141414] border border-white/5 p-5">
              ...
              <div className="flex gap-6 flex-wrap mb-3">
                ...
                <div>
                  ...
                  <p className="text-white/20 text-[10px]">Released after 3 days</p>
                </div>
                <div>
              ...
              <>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    ...
                    { label: 'Products', value: analytics.product_count || myProducts.length, color: 'text-green-300' },
                        ...
                        {(() => {
                          ...
                          return analytics.daily_sales.map((d, i) => (
                            <div key={i} className="flex-1 flex flex-col items-center gap-1 group relative">
                              ...
                              <div className="w-full bg-gradient-to-t from-cyan-500 to-purple-500 rounded-t"
                ...
                {analytics.recent_withdrawals && analytics.recent_withdrawals.length > 0 && (
                  <Card className="bg-[#141414] border border-white/5">
                    <CardContent className="p-6">
      ...
      <Dialog open={productDialog} onOpenChange={(o) => { setProductDialog(o); if (!o) resetProductForm(); }}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto bg-gray-900 border-white/20">
          <DialogHeader><DialogTitle className="text-white">{editingProduct ? 'Edit Product' : 'Add New Product'}</DialogTitle></DialogHeader>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Select value={productForm.category} onValueChange={(v) => setProductForm(p => ({ ...p, category: v }))}>
                  <SelectTrigger className="bg-white/10 border-white/20 text-white"><SelectValue placeholder="Choose type" /></SelectTrigger>
                  <SelectContent>{approvedCategories.map(c => <SelectItem key={c} value={c}>{getCatMeta(c).label}</SelectItem>)}</SelectContent>
                </Select>
                {approvedCategories.length === 0 && <p className="text-red-400 text-xs mt-1">Request category access first</p>}
              </div>
