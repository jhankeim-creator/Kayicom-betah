...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const allowedOrigins = (process.env.VISUAL_EDIT_ALLOWED_ORIGINS || "")
      .split(",")
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@kayicom.com" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@kayicom.com" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
